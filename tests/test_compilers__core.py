...
def tests_get_compiler():
    ...
    from compilertools.compilers._core import get_compiler, _which_unix_compiler
    ...
    # Return compiler by name with all file in "compilertools.compilers"
...
def tests_compiler_base():
    ...
    class Compiler(CompilerBase):
        ...
        def _compile_args_matrix(self, arch, cpu):
            ...
            return [
                ...
                [
                    self.Arg(
                        args="--arch1", suffix="arch1", import_if=(arch == "arch1")
                    ),
                    self.Arg(
                        args="--arch2", suffix="arch2", import_if=(arch == "arch2")
                    ),
                ],
            ]
    ...
    assert _order_args_matrix(matrix, current_machine=True) == excepted_currentmachine
    assert _order_args_matrix(matrix, current_compiler=True) == excepted_currentcompiler

    # Test compile_args
    assert compiler1.compile_args(arch="arch1") == excepted
    assert (
        compiler1.compile_args(arch="arch1", current_machine=True)
        == excepted_currentmachine
    )
    assert (
        Compiler(current_compiler=True).compile_args(arch="arch1")
        == excepted_currentcompiler
    )

    # Test compile_args_current_machine
    assert compiler2.compile_args_current_machine() == excepted["inst1-arch1"]
