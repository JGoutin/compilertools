...
def tests_suffix_from_args():
    ...
    # Default
    assert suffix_from_args(args) == [".suffix1", ".suffix2"]

    # With extension
    assert suffix_from_args(args, ".pyd") == [".suffix1.pyd", ".suffix2.pyd"]
...
def tests_log_exception(caplog):
    ...
    try:
        ...
        assert message in "".join((rec.message for rec in caplog.records))
