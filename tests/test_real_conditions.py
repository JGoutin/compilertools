...
def _build_and_import(
    setup, extension, ext_function=None, source_ext="module.c", source_content=C_SOURCE
):
    """Test build with true files and compiler, and then import generated files."""
    ...
    with TemporaryDirectory() as tmp:
        ...
        # Handle setup errors
            ...
            if (
                "Unable to find vcvarsall.bat" in message
                or
                # Or not found by Setuptools
                ("Microsoft Visual C++ " in message and " is required." in message)
            ):
        ...
        for suffix in ARCH_SUFFIXES:
            ...
            process = Popen([executable, script], stdout=PIPE, stderr=PIPE, shell=False)
...
def tests_build_cython():
    ...
    _build_and_import(
        setup,
        Extension,
        ext_function=cythonize,
        source_ext=".pyx",
        source_content=PYX_SOURCE,
    )
